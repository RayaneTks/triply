...
async function fetchGeocodeFirst(keyword: string): Promise<{ lat: number; lng: number; name: string } | null> {
    ...
    try {
        const res = await fetch(
            `/api/v1/integrations/amadeus/places?${new URLSearchParams({ keyword: q, subType: 'CITY,AIRPORT' }).toString()}`,
        );
...
async function resolveDestinationLabels(params: {
    ...
    if (iata.length >= 3) {
        try {
            const res = await fetch(
                `/api/v1/integrations/amadeus/places?${new URLSearchParams({ keyword: iata, subType: 'CITY,AIRPORT' }).toString()}`,
            );
            const data: unknown = await res.json();
            const list = Array.isArray(data) ? data : [];
...
export function HomePage({ forceAppStart = false }: { forceAppStart?: boolean } = {}) {
    ...
    const searchHotelsAtLocation = useCallback(async (lat: number, lng: number, cityCenter: LocationPoint | null, ratingsFilter?: number[] | null) => {
        ...
        try {
            const url = new URL('/api/v1/integrations/amadeus/hotels/by-geocode', window.location.origin);
    ...
    const handleFlightSearch = async (options?: { autoSelect?: boolean }) => {
        ...
        try {
            ...
            const res = await fetch('/api/v1/integrations/amadeus/flights/search', {
                method: 'POST',
    ...
    const handleHotelSearch = async (options?: { autoSelect?: boolean }) => {
        ...
        try {
            const res = await fetch('/api/v1/integrations/amadeus/hotels/search', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    cityCode: city,
    ...
    const focusMapOnDestination = useCallback(async () => {
        ...
        if (destinationGeo && destinationGeo.iataCode === iata) {
            ...
            try {
                const res = await fetch(
                    `/api/v1/integrations/amadeus/places?${new URLSearchParams({ keyword, subType: 'CITY,AIRPORT' }).toString()}`,
                );
                const data: unknown = await res.json();
                const list = Array.isArray(data) ? data : [];
                const first = list[0] as
