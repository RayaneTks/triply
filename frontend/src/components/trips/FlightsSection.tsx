...
export function FlightsSection({
    ...
    const handleSearch = useCallback(async () => {
        ...
        try {
            ...
            if (!dep || !isoDate.test(dep)) {
                setApiResponse({
                    ...emptyEnv,
                    error: 'Indiquez une date de d√©part (aller) au format AAAA-MM-JJ.',
            ...
            if (!originCode || !destCode) {
                setApiResponse({ ...emptyEnv, error: `Aucun a√©roport IATA trouv√© pour "${!originCode ? departureCity : arrivalCity}". Essayez un code 3 lettres (CDG, BCN‚?¶).` } as unknown as AmadeusResponse);
                return;
            }
            const body = {
    ...
    const handleSelectOffer = useCallback(async (offer: FlightOffer, carrierName: string) => {
        ...
        try {
            ...
            setOpMessage('Vol ajout√© au voyage.');
            setModalOpen(false);
            setApiResponse(null);
            await reload();
        } catch (err) {
    ...
    const handleBook = useCallback(async (flight: FlightRecord) => {
        try {
            ...
            setOpMessage(err instanceof Error ? err.message : 'Lien de r√©servation indisponible.');
        }
    ...
    const headerSubtitle = useMemo(() => {
        ...
        if (flights.length === 0) return 'Aucun vol s√©lectionn√©';
        return `${flights.length} vol${flights.length > 1 ? 's' : ''} enregistr√©${flights.length > 1 ? 's' : ''}`;
            ...
            {loading ? (
                ...
                <div className="triply-card p-8 text-center space-y-3">
                    <p className="text-sm text-light-muted font-bold">
                        Aucun vol enregistr√© pour ce voyage.
                    </p>
