...
- **`frontend/triplydev/` (Next.js App Router)** — Primary frontend application in dev and production. API calls are proxied to Laravel via Next rewrites and environment variables (`BACKEND_PROXY_TARGET`, `BACKEND_API_BASE_URL`).
- **`frontend/triplydev/`** — Next.js app used for some pages and CI (lint/build); not the default Docker web entry for the trip planner.
- **`frontend/triply-docs-lib/`** — Shared React component library and Storybook.
